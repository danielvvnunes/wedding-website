...
function localEmailApi() {
  return {
    ...
    configureServer(server) {
      ...
      for (const key of ["RESEND_API_KEY", "RESEND_MANAGEMENT_API_KEY", "RESEND_FROM_EMAIL", "RESEND_REPLY_TO", "TABLE_EMAIL_ADMIN_PASSWORD", "ADMIN_PASSWORD", "VITE_ADMIN_PASSWORD", "SUPABASE_URL", "VITE_SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"]) {
        if (process.env[key] === undefined && env[key] !== undefined) process.env[key] = env[key];
      }
      server.middlewares.use(async (req, res, next) => {
        const path = req.url?.split("?")[0];
        if (!["/api/table-email-campaign", "/api/send-table-emails", "/api/reminder-email-campaign"].includes(path)) return next();
        ...
        try {
          ...
          const { default: handler } = await server.ssrLoadModule(path === "/api/reminder-email-campaign" ? "/api/reminder-email-campaign.js" : "/api/table-email-campaign.js");

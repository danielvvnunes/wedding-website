...
test('gallery moderation rejects public credentials and resolves deletion paths on server', async t => {
 ...
 const original=globalThis.fetch; const calls=[]; let failStorage=false;
 ...
 globalThis.fetch=async (input,init)=>{
  ...
  return new Response(JSON.stringify(r.method==='GET'?{id:'test-id',file_path:'guest/original.jpg',file_type:'image/jpeg'}:null),{headers:{'Content-Type':'application/json'}});
 };
